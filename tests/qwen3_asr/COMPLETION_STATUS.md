...
| Three Chinese fixture categories | Local macOS TTS fixtures exist for short Mandarin, long Mandarin, and Mandarin mixed with English/numbers/punctuation. | Replace or supplement with real Mandarin recordings when available. | Run the same fixtures through the original PyTorch Qwen3-ASR pipeline. |
...
| Module-level localization | ncnn result JSON records audio embedding summary. Older experiments already compared exported modules, but the new ASR-level schema still lacks projector/adaptor and first decoder logits summaries. | Add ncnn summaries for first decoder logits and any separable projector/adaptor output. | Add PyTorch summaries for speech encoder, projector/adaptor, and first decoder logits; compare `max_abs`, `mean_abs`, `p99_abs`, cosine similarity, and top-k agreement. |
| Multi-platform smoke | macOS CPU-only build and smoke are available. Result JSON records threads, total time, RTF, chunk count, and chunking strategy. | Keep macOS as correctness/output-contract smoke, not performance proof. | Add Linux and Windows build/smoke. For Vulkan/ARM/mobile, record a separate performance table and keep it separate from PyTorch accuracy parity. |
